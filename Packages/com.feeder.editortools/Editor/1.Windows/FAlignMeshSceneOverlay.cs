...
namespace Feeder
{
    ...
    public static class FAlignMeshSceneOverlay
    {
        ...
        private static bool ApplyAutoAlignToHighlightDrawer()
        {
            ...
            FMeshAutoAlignUtils.AutoAlignOptions options = FMeshAutoAlignUtils.AutoAlignOptions.Default;
            bool success = FMeshAutoAlignUtils.TryAutoAlign(
                meshGizmo,
                meshB,
                tB,
                tB.lossyScale,
                options,
                out FMeshAutoAlignUtils.AutoAlignResult result);
        ...
        private static void ApplyTrimIcpToHighlightDrawer()
        {
            ...
            for (int iter = 0; iter < icpMaxIterations; iter++)
            {
                ...
                if (!FMeshMatchTransformUtils.ComputeRigidTransform(worldSource, pairedTarget, out Matrix4x4 Rt))
                {
